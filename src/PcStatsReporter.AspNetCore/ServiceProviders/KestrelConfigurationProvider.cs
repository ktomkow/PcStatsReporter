...
public static class KestrelConfigurationProvider
{
    public static void ConfigureKestrel(this WebApplicationBuilder builder)
    {
        builder.WebHost.ConfigureKestrel(options =>
        {
            ...
            // Setup a HTTP/1 endpoint without TLS for grpc
            options.ListenAnyIP(ports.First(), o => o.Protocols =
                HttpProtocols.Http1);
    
            // Setup a HTTP/2 endpoint without TLS for web clients
            options.ListenAnyIP(ports.Last(), o => o.Protocols =
                HttpProtocols.Http2);
        });
    }
}